...
export function DietarySection({ assessment }: { assessment: any }) {
  ...
  const tDiet = useTranslations("onboarding.assessment");
  ...
  return (
    <div className="border-border bg-card overflow-hidden rounded-xl border">
      <div className="border-border flex items-center gap-2 border-b bg-neutral-50/50 p-4">
        ...
        <h3 className="text-sm font-semibold">{t("dietaryTitle")}</h3>
      </div>
      <div className="space-y-4 p-4">
        <div>
          <label className="mb-1.5 block text-xs font-medium">{t("foodPreferences")}</label>
        ...
        <div>
          <label className="mb-1.5 block text-xs font-medium">{t("allergies")}</label>
          <div className="flex flex-wrap gap-2">
            {ALLERGY_IDS.map((id) => (
              <button
                key={id}
        ...
        <div>
          <label className="mb-1.5 block text-xs font-medium">{t("dietaryRestrictions")}</label>
          <div className="flex flex-wrap gap-2">
