...
export function GoalsSection({ assessment }: { assessment: any }) {
  ...
  const tAssessment = useTranslations("onboarding.assessment");
  ...
  return (
    <div className="border-border bg-card overflow-hidden rounded-xl border">
      <div className="border-border flex items-center gap-2 border-b bg-neutral-50/50 p-4">
        ...
        <h3 className="text-sm font-semibold">{t("goalsTitle")}</h3>
      </div>
      <div className="space-y-4 p-4">
        <div>
          <label className="mb-1.5 block text-xs font-medium">{t("primaryGoal")}</label>
          <div className="flex flex-col gap-2">
            {GOAL_IDS.map((id) => (
              ...
              >
                {tAssessment(`primaryGoals.${id}`)}
              </button>
            ))}
          </div>
        </div>
        <div>
          <label className="mb-1.5 block text-xs font-medium">{t("activityLevel")}</label>
          <select
            value={activityLevel}
          ...
          >
            {ACTIVITY_LEVELS.map((id) => (
              <option key={id} value={id}>
                {tAssessment(`activityLevels.${id}`)}
              </option>
            ))}
          </select>
        </div>
        <div>
          <label className="mb-1.5 block text-xs font-medium">{t("experienceLevel")}</label>
          <select
