...
export function MedicalSection({ assessment }: { assessment: any }) {
  ...
  const tMedical = useTranslations("onboarding.assessment");
    ...
    <>
      <div className="border-border bg-card overflow-hidden rounded-xl border">
        <div className="border-border flex items-center gap-2 border-b bg-neutral-50/50 p-4">
          ...
          <h3 className="text-sm font-semibold">{t("medicalTitle")}</h3>
        </div>
        <div className="space-y-4 p-4">
          <div>
            <label className="mb-1.5 block text-xs font-medium">{t("menstrualStatus")}</label>
            <select
            ...
            >
              ...
              {MENSTRUAL_OPTIONS.map((opt) => (
                <option key={opt.value} value={opt.value}>
                  {tMedical(`femaleHealth.${opt.key}`)}
                </option>
              ))}
            </select>
          </div>
          <div className="flex items-center justify-between">
            <label className="text-sm font-medium">{t("pregnancy")}</label>
            <button
              type="button"
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium">{t("hormonalMedication")}</label>
            <input
              type="text"
          ...
          <div className="bg-card w-full max-w-sm rounded-2xl shadow-xl">
            <div className="space-y-3 p-5">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold">{t("safetyWarning")}</h3>
              </div>
              <p className="text-muted-foreground text-xs">{t("safetyWarningDesc")}</p>
