...
export function WeightStep({ inBodyFile, onInBodyFileChange }: WeightStepProps) {
  ...
  return (
    <div className="space-y-4">
      <SectionCard icon={Weight} title={t("checkIn.weight")}>
        <div className="flex items-center gap-3">
          <Input
            ...
            error={!!errors.weight}
          />
          <span className="text-muted-foreground font-semibold">{t("units.kg")}</span>
        </div>
      ...
      <div className="flex gap-2">
        ...
        >
          {t("checkIn.manualMeasurements")}
          <span className="text-muted-foreground ms-1 text-xs font-normal">
            ({t("checkIn.recommended")})
          </span>
        </button>
        <button
          type="button"
      ...
      {measurementMethod === "manual" && (
        <SectionCard
          title={`${t("checkIn.measurements")} (${t("checkIn.optional")})`}
          description={`${t("checkIn.allIn")} ${t("units.cm")}`}
          variant="neutral"
        >
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              { key: "chest" as const, label: t("checkIn.chest") },
              { key: "waist" as const, label: t("checkIn.waist") },
              { key: "hips" as const, label: t("checkIn.hips") },
              { key: "arms" as const, label: t("checkIn.arms") },
              { key: "thighs" as const, label: t("checkIn.thighs") },
            ].map((m) => (
      ...
      {measurementMethod === "inbody" && (
        <SectionCard
          title={t("checkIn.inBodyUpload")}
          description={t("checkIn.uploadInBodyDescription")}
          variant="neutral"
        >
          <input
            type="file"
          ...
          {!inBodyFile ? (
            ...
            >
              ...
              <p className="text-sm font-semibold">{t("checkIn.uploadInBody")}</p>
              <p className="text-muted-foreground mt-1 text-xs">
                {t("checkIn.fileTypes", { maxFileMB: MAX_UPLOAD_SIZE_MB })}
              </p>
            </label>
          ) : (
            <div className="border-border relative overflow-hidden rounded-xl border">
              <img
                ...
                alt={t("checkIn.inBodyResult")}
                className="max-h-64 w-full object-contain"
              />
              <button
                type="button"
                onClick={() => onInBodyFileChange(null)}
                aria-label={t("checkIn.removeFile")}
