...
export function MeasurementsSection({
  ...
  const measurementFields = [
    { key: "chest", value: chest, setter: setChest, label: t("assessment.chest") },
    { key: "waist", value: waist, setter: setWaist, label: t("assessment.waist") },
    { key: "hips", value: hips, setter: setHips, label: t("assessment.hips") },
    { key: "arms", value: arms, setter: setArms, label: t("assessment.arms") },
    { key: "thighs", value: thighs, setter: setThighs, label: t("assessment.thighs") },
  ];
  ...
  return (
    <div className="space-y-4">
      <SectionCard
        icon={Ruler}
        title={t("assessment.measurementsTitle")}
        description={t("assessment.measurementsDesc")}
      >
        {/* Method toggle */}
        <div className="flex gap-2">
          ...
          >
            {t("assessment.manualMeasurements")}
      ...
      {/* Manual measurements */}
      {measurementMethod === "manual" && (
        <SectionCard title={t("assessment.allInCm")} variant="neutral">
      ...
      {measurementMethod === "inbody" && (
        <SectionCard
          title={t("assessment.inBodyUpload")}
          description={t("assessment.uploadInBodyDescription")}
          variant="neutral"
        >
          <p className="text-muted-foreground mb-3 text-xs">{t("assessment.manualRecommended")}</p>
          ...
          {!inBodyFile ? (
            ...
            >
              ...
              <p className="text-sm font-semibold">{t("assessment.uploadInBody")}</p>
              <p className="text-muted-foreground mt-1 text-xs">
                {t("assessment.fileTypes", { maxFileMB: MAX_UPLOAD_SIZE_MB })}
              </p>
            </label>
          ) : (
            <div className="border-border relative overflow-hidden rounded-xl border">
              <img
                ...
                alt={t("assessment.inBodyResult")}
                className="max-h-64 w-full object-contain"
              />
              <button
                type="button"
                onClick={() => onInBodyFileChange(null)}
                aria-label={t("assessment.removeFile")}
                disabled={isLoading}
