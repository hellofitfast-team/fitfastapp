...
import { ActivityCard } from "@/components/clients/activity-card";
import { SupportCard } from "@/components/clients/support-card";
import { CheckInHistoryCard } from "@/components/clients/checkin-history-card";
import { ProgressCard } from "@/components/clients/progress-card";
import { AssessmentHistoryCard } from "@/components/clients/assessment-history-card";
import { WorkoutLogCard } from "@/components/clients/workout-log-card";
...
function AssessmentCard({
  ...
  const { t, i18n } = useTranslation();
  const locale = i18n.language;
  // tA uses t("assessmentDetails.xxx") pattern
    ...
    <div className="rounded-xl border border-stone-200 bg-white p-5">
      ...
      {assessment ? (
        <div>
          ...
          >
            {expanded ? t("assessmentDetails.showLess") : t("assessmentDetails.showMore")}
            <ChevronDown
          ...
          {expanded && (
            <dl className="mt-2 space-y-2.5 border-t border-stone-100 pt-3 text-sm">
              {assessment.age != null && (
                <div className="flex justify-between">
                  <dt className="text-stone-500">{t("assessmentDetails.age")}</dt>
                  <dd className="text-stone-900">{assessment.age as number}</dd>
                </div>
              )}
              {assessment.gender != null && (
                <div className="flex justify-between">
                  <dt className="text-stone-500">{t("assessmentDetails.gender")}</dt>
                  <dd className="text-stone-900 capitalize">{assessment.gender as string}</dd>
                </div>
              )}
              {assessment.activityLevel != null && (
                <div className="flex justify-between">
                  <dt className="text-stone-500">{t("assessmentDetails.activityLevel")}</dt>
                  <dd className="text-stone-900 capitalize">
              ...
              {assessment.exerciseHistory != null && (
                <div className="flex justify-between">
                  <dt className="text-stone-500">{t("assessmentDetails.exerciseHistory")}</dt>
              ...
              {assessment.scheduleAvailability != null && (
                <div>
                  <dt className="mb-1 text-stone-500">{t("assessmentDetails.schedule")}</dt>
              ...
              {assessment.measurements != null && (
                <div>
                  <dt className="mb-1 text-stone-500">{t("assessmentDetails.measurements")}</dt>
              ...
              {(assessment.foodPreferences as string[] | undefined)?.length ? (
                <div>
                  <dt className="mb-1 text-stone-500">{t("assessmentDetails.foodPreferences")}</dt>
              ...
              {(assessment.allergies as string[] | undefined)?.length ? (
                <div>
                  <dt className="mb-1 text-stone-500">{t("assessmentDetails.allergies")}</dt>
                  <dd className="flex flex-wrap gap-1.5">
              ...
              {(assessment.dietaryRestrictions as string[] | undefined)?.length ? (
                <div>
                  <dt className="mb-1 text-stone-500">
                    {t("assessmentDetails.dietaryRestrictions")}
                  </dt>
                  <dd className="flex flex-wrap gap-1.5">
              ...
              {(assessment.medicalConditions as string[] | undefined)?.length ? (
                <div>
                  <dt className="mb-1 text-stone-500">
                    {t("assessmentDetails.medicalConditions")}
                  </dt>
                  <dd className="flex flex-wrap gap-1.5">
              ...
              {(assessment.injuries as string[] | undefined)?.length ? (
                <div>
                  <dt className="mb-1 text-stone-500">{t("assessmentDetails.injuries")}</dt>
                  <dd className="flex flex-wrap gap-1.5">
              ...
              {assessment.lifestyleHabits != null && (
                <div>
                  <dt className="mb-1 text-stone-500">{t("assessmentDetails.lifestyle")}</dt>
                  <dd className="rounded-lg bg-stone-50 p-2.5 text-xs text-stone-700">
                    {(() => {
                      ...
                      const parts: string[] = [];
                      if (habits.equipment)
                        parts.push(`${t("assessmentDetails.equipment")}: ${habits.equipment}`);
                      if (habits.mealsPerDay)
                        parts.push(`${t("assessmentDetails.mealsPerDay")}: ${habits.mealsPerDay}`);
                      return parts.join(" · ") || "---";
                    })()}
                  </dd>
                </div>
              ...
              {assessment.femaleHealth != null && (
                <div>
                  <dt className="mb-1 text-stone-500">{t("assessmentDetails.femaleHealth")}</dt>
                  <dd className="rounded-lg bg-pink-50 p-2.5 text-xs text-pink-700">
                    {(() => {
                      ...
                      if (fh.menstrualStatus) {
                        ...
                        parts.push(
                          `${t("assessmentDetails.menstrualStatus")}: ${t(`assessmentDetails.${statusKey}`)}`,
                        );
                      }
                      if (fh.isPregnant) parts.push(`✓ ${t("assessmentDetails.pregnant")}`);
                      if (fh.isBreastfeeding)
                        parts.push(`✓ ${t("assessmentDetails.breastfeeding")}`);
                      if (fh.hormonalMedication)
                        parts.push(
                          `${t("assessmentDetails.hormonalMedication")}: ${fh.hormonalMedication}`,
                        );
...
export const Route = createFileRoute("/_panel/clients/$id")({
  ...
  pendingComponent: () => <TableSkeleton rows={6} cols={3} />,
...
function ClientDetailPage() {
  ...
  // tNotif uses t("notifications.xxx") pattern
  ...
  const handleSendNotification = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: t("notifications.sentDesc"),
        variant: "success",
      });
        ...
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            >
              ...
              {isSendingNotif ? t("notifications.sending") : t("notifications.confirmSend")}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
