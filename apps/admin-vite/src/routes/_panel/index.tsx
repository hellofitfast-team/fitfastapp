  ...
  return (
    ...
    >
      ...
      <div className="group-hover:text-primary mt-3 flex items-center gap-1 text-xs font-medium text-stone-400 transition-colors">
        {t("admin.view")} <ArrowRight className="h-3 w-3 rtl:rotate-180" />
      </div>
    </Link>
  );
}
    ...
    <Card className="chart-section">
      <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
        <div>
          ...
          >
            {t("admin.clientGrowth") || "Client Growth"}
          </h2>
          <p className="mt-0.5 text-xs text-stone-500">
            {t("admin.lastSixMonths") || "Last 6 months"}
          </p>
        </div>
      ...
      <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
        <div>
          <h2
            ...
            style={{ fontFamily: "var(--font-display)" }}
          >
            {t("admin.weeklyActivity") || "Weekly Activity"}
          </h2>
          <p className="mt-0.5 text-xs text-stone-500">
            {t("admin.lastFourWeeks") || "Last 4 weeks"}
          </p>
        </div>
        <div className="bg-primary/10 text-primary flex h-8 w-8 items-center justify-center rounded-lg">
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        ".chart-section",
        { y: 50, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.7, stagger: 0.15, ease: "power3.out", delay: 0.4 },
      );
      gsap.fromTo(
        ".quick-action",
        { y: 30, opacity: 0 },
  ...
  return (
    <div ref={containerRef} className="space-y-8">
      <div className="page-title">
        ...
        <p className="mt-1 text-sm text-stone-500">{t("admin.overview")}</p>
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label={t("admin.totalClients")} value={totalClients} icon={Users} to="/clients" />
        <StatCard
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="quick-action">
          <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
            <h2 className="font-semibold text-stone-900">{t("admin.pendingSignups")}</h2>
            <Link
              to="/signups"
          ...
          <CardContent>
            {pendingSignupsCount === 0 ? (
              <p className="text-sm text-stone-400">{t("admin.noActivity")}</p>
            ) : (
              <p className="text-sm font-medium text-red-600">
                {pendingSignupsCount} {t("admin.pendingSignups").toLowerCase()}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="quick-action">
          <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
            <h2 className="font-semibold text-stone-900">{t("admin.openTickets")}</h2>
            <Link
              to="/tickets"
              className="link-lift text-primary hover:text-primary text-xs font-medium transition-colors"
            >
          ...
          <CardContent>
            {openTicketsCount === 0 ? (
              ...
              <p className="text-sm font-medium text-red-600">
                {openTicketsCount} {t("admin.openTickets").toLowerCase()}
              </p>
            )}
          </CardContent>
        </Card>
