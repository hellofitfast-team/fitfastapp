...
export function TicketsList() {
  ...
  const handleRespond = async (ticketId: Id<"tickets">) => {
    ...
    try {
      ...
      toast({ title: t("admin.respondFailed"), variant: "destructive" });
    }
  ...
  const handleClose = async (ticketId: Id<"tickets">) => {
    ...
    try {
      ...
      toast({ title: t("admin.closeFailed"), variant: "destructive" });
    } finally {
      setClosingId(null);
    }
  };
  ...
  if (tickets.length === 0)
    return (
      <div className="rounded-xl border border-stone-200 bg-white p-12 text-center">
        ...
        <p className="font-medium text-stone-500">{t("admin.noResults")}</p>
      </div>
    );
      ...
      {tickets.map((ticket) => {
        ...
        return (
          ...
          >
            ...
            >
              <div
                className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${statusStyles[ticket.status] ?? ""}`}
              >
              ...
              <span
                className={`shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-medium ${statusStyles[ticket.status] ?? ""}`}
              >
                {t(`admin.ticketStatuses.${ticket.status}`)}
              </span>
            </button>
            {isExpanded && (
                ...
                {ticket.messages.map((msg, i) => (
                  ...
                  >
                    <p className="mb-1 text-xs font-medium text-stone-400">
                      {msg.sender === "coach" ? t("admin.coachResponse") : t("admin.client")}
                    </p>
