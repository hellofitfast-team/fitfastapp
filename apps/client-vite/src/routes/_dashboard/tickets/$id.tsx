...
function groupMessagesByDate(
  ...
  for (const msg of messages) {
    ...
    if (existing) {
      ...
      let label: string;
      if (dateKey === today) {
        label = t("tickets.chat.today");
      } else if (dateKey === yesterday) {
        label = t("tickets.chat.yesterday");
      } else {
...
function TicketDetailPage() {
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      toast({ title: t("tickets.errors.replyFailed"), variant: "destructive" });
    } finally {
      setIsSending(false);
    }
  };
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-6 py-4 lg:px-6 lg:py-6">
        ...
        <div className="border-border bg-card rounded-xl border p-10 text-center">
          ...
          <p className="font-medium">{t("tickets.ticketNotFound")}</p>
        </div>
      </div>
    );
  }
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              {ticket.category && (
                <span className="text-muted-foreground text-xs">
                  {safeT(`tickets.categories.${toCamelCase(ticket.category)}`, ticket.category)}
                </span>
              )}
            </div>
          </div>
      ...
      {ticket.status !== "closed" ? (
        <div className="bg-background border-border sticky bottom-0 -mx-4 flex gap-2 border-t p-3 px-4 lg:-mx-6 lg:px-6">
          ...
          <button
            ...
            aria-label={t("tickets.sendReply")}
        ...
        <div className="border-border rounded-lg border bg-neutral-50 p-3 text-center">
          <p className="text-muted-foreground text-sm">{t("tickets.ticketClosed")}</p>
        </div>
      )}
    </div>
  );
