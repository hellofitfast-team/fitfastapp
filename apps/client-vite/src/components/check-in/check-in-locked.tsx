    ...
    <div className="border-border bg-card shadow-card overflow-hidden rounded-xl border">
      <div className="border-border bg-primary/5 flex items-center gap-3 border-b p-4">
        ...
        <h2 className="text-sm font-semibold">
          {t("checkIn.checkInLocked", { defaultValue: "Check-In Locked" })}
        </h2>
      </div>
      <div className="p-8 text-center">
        ...
        <h3 className="text-lg font-bold">
          {t("checkIn.nextCheckInAvailable", { defaultValue: "Next Check-In Available" })}
        </h3>
        <p className="text-primary mt-3 text-4xl font-bold">
          {toLocalDigits(daysUntilNextCheckIn, locale)}{" "}
          {t("checkIn.days", { defaultValue: "days" })}
        </p>
        <p className="text-muted-foreground mt-3 text-sm">
          {nextCheckInDate.toLocaleDateString(toDateLocale(locale), {
            weekday: "long",
            year: "numeric",
            month: "long",
            day: "numeric",
        ...
        <div className="border-border mt-6 rounded-lg border border-dashed bg-neutral-50 p-4">
          <p className="text-muted-foreground text-sm">
            {t("checkIn.lockReason", { days: frequencyDays })}
          </p>
        </div>
      </div>
    </div>
