  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard
        ...
        title={t("assessment.cuisineTitle")}
        description={t("assessment.cuisineDesc")}
        variant="nutrition"
      >
        <MultiSelect
          options={CUISINE_PREFERENCES.map((item) => ({
            ...item,
            label: t(`assessment.cuisines.${item.id}`),
          }))}
          ...
          otherLabel={t("assessment.otherOption")}
          otherPlaceholder={t("assessment.specifyPlaceholder")}
        />
      </SectionCard>
      ...
      <SectionCard
        icon={Clock}
        title={t("assessment.mealsPerDayTitle")}
        description={t("assessment.mealsPerDayDesc")}
        variant="nutrition"
      >
        <div className="grid grid-cols-2 gap-2">
          {MEALS_PER_DAY.map((option) => {
            ...
            return (
              ...
              >
                {t(`assessment.mealsPerDayOptions.${option.id}`)}
              </button>
            );
          })}
        </div>
      ...
      {/* Food Allergies */}
      <SectionCard icon={ShieldAlert} title={t("assessment.allergiesTitle")} variant="nutrition">
        <MultiSelect
          options={COMMON_ALLERGIES.map((item) => ({
            ...item,
            label: t(`assessment.allergies.${item.id}`),
          }))}
      ...
      <SectionCard icon={Ban} title={t("assessment.restrictionsTitle")} variant="nutrition">
        <MultiSelect
          options={DIETARY_RESTRICTIONS.map((item) => ({
            ...item,
            label: t(`assessment.restrictions.${item.id}`),
          }))}
