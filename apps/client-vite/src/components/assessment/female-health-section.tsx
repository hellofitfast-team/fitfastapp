...
export function FemaleHealthSection({ data, onChange, isLoading }: FemaleHealthSectionProps) {
  ...
  return (
    <SectionCard
      icon={Heart}
      title={t("assessment.femaleHealth.title")}
      description={t("assessment.femaleHealth.description")}
      variant="neutral"
    >
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium">
            {t("assessment.femaleHealth.menstrualStatus")}
          </label>
          <div className="flex flex-wrap gap-2">
            {MENSTRUAL_OPTIONS.map((option) => (
              ...
              >
                {t(`assessment.femaleHealth.${option}`)}
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Pregnancy */}
        <div className="space-y-2">
          <label className="text-sm font-medium">{t("assessment.femaleHealth.pregnant")}</label>
          <div className="flex gap-2">
            {[true, false].map((val) => (
              ...
              >
                {val ? t("assessment.femaleHealth.yes") : t("assessment.femaleHealth.no")}
              </button>
            ))}
          </div>
          {data.isPregnant && (
            <div className="mt-2 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
              ...
              <p className="text-xs text-amber-700">
                {t("assessment.femaleHealth.pregnancyWarning")}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium">
            {t("assessment.femaleHealth.breastfeeding")}
          </label>
          <div className="flex gap-2">
            {[true, false].map((val) => (
              <button
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium">
            {t("assessment.femaleHealth.hormonalMedication")}
          </label>
          <Input
            placeholder={t("assessment.femaleHealth.hormonalMedicationPlaceholder")}
        ...
        {/* Additional notes */}
        <div className="space-y-2">
          <label className="text-sm font-medium">{t("assessment.femaleHealth.notes")}</label>
          <Textarea
            placeholder={t("assessment.femaleHealth.notesPlaceholder")}
            value={data.notes ?? ""}
