...
export function ScheduleSection({
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard
        icon={Calendar}
        title={t("assessment.scheduleTitle")}
        description={t("assessment.scheduleDesc")}
        variant="routine"
      >
        <div className="space-y-3">
          ...
          <div className="text-muted-foreground flex justify-between px-1 text-[10px]">
            {DAYS.map((day) => (
              <span key={day.id}>{t(`assessment.days.${day.id}`)}</span>
            ))}
          </div>
      ...
      <SectionCard
        icon={Clock}
        title={t("assessment.sessionDurationTitle")}
        description={t("assessment.sessionDurationDesc")}
        variant="routine"
      >
        <div className="grid grid-cols-4 gap-2">
          {SESSION_DURATIONS.map((option) => {
            ...
            return (
              ...
              >
                {t(`assessment.sessionDurations.${option.id}`)}
              </button>
            );
          })}
        </div>
      ...
      <SectionCard
        icon={Sun}
        title={t("assessment.trainingTimeTitle")}
        description={t("assessment.trainingTimeDesc")}
        variant="routine"
      >
        <div className="grid grid-cols-2 gap-2">
          {TRAINING_TIMES.map((option) => {
            ...
            return (
              ...
              >
                {t(`assessment.trainingTimes.${option.id}`)}
              </button>
            );
          })}
        </div>
