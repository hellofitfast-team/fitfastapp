  ...
  return (
    <div className="border-border bg-card overflow-hidden rounded-xl border">
      <div className="border-border flex items-center gap-2 border-b bg-neutral-50/50 p-4">
        ...
        <h3 className="text-sm font-semibold">{t("assessment.scheduleTitle")}</h3>
      </div>
      <div className="space-y-4 p-4">
        <div>
          <label className="mb-1.5 block text-xs font-medium">{t("assessment.trainingDays")}</label>
          ...
          >
            {TIMES.map((id) => (
              <option key={id} value={id}>
                {tSchedule(`assessment.trainingTimes.${id}`)}
              </option>
            ))}
          </select>
        </div>
